...
- **`/story` returns 502 "Gemini is not reachable" even though the key is valid**: the pinned model id in `app/services/gemini_service.py` (`GEMINI_MODEL`) may have been retired by Google. A discontinued model returns `NOT_FOUND`, which `call_gemini` catches as `genai_errors.APIError` and re-raises as the generic 502 — so a retired model looks identical to a bad key or a quota block. Confirm which it is with a direct call (`curl ".../v1beta/models/<id>:generateContent?key=KEY" ...`); if it's `NOT_FOUND`, re-pin `GEMINI_MODEL` to the current flash-lite id from <https://ai.google.dev/gemini-api/docs/models> and redeploy. Current pin: `gemini-3.1-flash-lite` (succeeded the retired `gemini-2.5-flash-lite`). Google emails a ~2-week deprecation notice before removing a model.
